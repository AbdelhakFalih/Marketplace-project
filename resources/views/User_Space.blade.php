...
<head>
    ...
    <title>@if(isset($role)) {{ ucfirst($role) }} {{ __('userDashboard') }} @else {{ __('adminDashboard') }} @endif</title>
</head>
<body>
<header>
    @include('Components', ['compo' => 'Menu User-space'])
</header>

<div class="header">
    <h1 data-i18n="welcome">Bienvenue sur votre @if($role === 'admin') {{ __('adminDashboard') }} @else {{ __('userDashboard') }} {{ ucfirst($role) }} @endif</h1>
</div>

@if($role == 'admin')
    <div class="dashboard">
        @include('Components', ['compo' => 'sidebar admin'])
        <div class="dashboard-content">
                        ...
                        <div class="col">
                            <div class="card h-100">
                                ...
                                <div class="card-body">
                                    ...
                                    <a href="/Detail?id={{ $user->id }}" class="btn btn-primary" data-i18n="viewDetails">Voir Détails</a>
                                    <a href="/Delete?id={{ $user->id }}" class="btn btn-danger" data-i18n="delete">Supprimer</a>
                                </div>
                            </div>
                        </div>
                    @endforeach
            ...
            <div class="container mt-5">
                ...
                <div class="row row-cols-1 row-cols-sm-2 row-cols-md-5 g-3">
                    <div class="col">
                        <div class="card h-100">
                            <img src="{{ asset('images/panier.png') }}" class="card-img-top" alt="Offre 1" style="height: 150px; object-fit: cover;">
                            <div class="card-body">
...
@elseif($role == 'cooperative' || $role == 'commerçant')
    <div class="dashboard">
        @include('Components', ['compo' => 'sidebar'])
        <div class="dashboard-content">
...
@else
    <div class="container mt-5 text-center">
        @include('Components', ['compo' => 'Errors_Page'], ['msg' => __('loginRequired')])
    </div>
@endif
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        var loginFailed = @json(isset($loginFailed) && $loginFailed);
        if (loginFailed) {
